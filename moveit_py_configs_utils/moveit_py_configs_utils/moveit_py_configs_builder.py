...
Example:
    moveit_configs = MoveItPyConfigsBuilder("robot_name").to_moveit_configs()
    ...
...
Each function in MoveItPyConfigsBuilder has a file_path as an argument which is used to override the default
path for the file

Example:
    moveit_configs = MoveItPyConfigsBuilder("robot_name")
...
class MoveItPyConfigsBuilder(ParameterBuilder):
    __moveit_configs: MoveItPyConfigs
    __robot_name: str
    __urdf_package: Path
    ...
    def __init__(
        ...
        self.__moveit_configs = MoveItPyConfigs(package_path=self._package_path)
        ...
        if setup_assistant_file.exists():
            ...
            if urdf_config := config.get("urdf", config.get("URDF")):
                if self.__urdf_package is None:
                    self.__urdf_package = Path(get_package_share_directory(urdf_config["package"]))
        ...
        if not self.__urdf_package or not self.__urdf_file_path:
            ...
            self.__urdf_package = self._package_path
            self.__urdf_file_path = self.__config_dir_path / (self.__robot_name + ".urdf")

        if not self.__srdf_file_path:
            logging.warning(
                f"\x1b[33;21mCannot infer SRDF from `{self._package_path}`. -- using config/{robot_name}.srdf\x1b[0m"
            )
            self.__srdf_file_path = self.__config_dir_path / (self.__robot_name + ".srdf")
    ...
    def robot_description(
        ...
        :return: Instance of MoveItPyConfigsBuilder with robot_description loaded.
        """
    ...
    def robot_description_semantic(
        ...
        :param mappings: mappings to be passed when loading the xacro file.
        :return: Instance of MoveItPyConfigsBuilder with robot_description_semantic loaded.
        """
        if (mappings is None) or all(
            (isinstance(key, str) and isinstance(value, str)) for key, value in mappings.items()
        ):
    ...
    def robot_description_kinematics(self, file_path: Optional[str] = None):
        ...
        :return: Instance of MoveItPyConfigsBuilder with robot_description_kinematics loaded.
        """
        self.__moveit_configs.robot_description_kinematics = {
            self.__robot_description
            + "_kinematics": load_yaml(
                self._package_path / (file_path or self.__config_dir_path / "kinematics.yaml")
            )
        }
        return self
    ...
    def joint_limits(self, file_path: Optional[str] = None):
        ...
        :return: Instance of MoveItPyConfigsBuilder with robot_description_planning loaded.
        """
        self.__moveit_configs.joint_limits = {
            self.__robot_description
            + "_planning": load_yaml(
                self._package_path / (file_path or self.__config_dir_path / "joint_limits.yaml")
            )
        }
        return self
    ...
    def moveit_cpp(self, file_path: Optional[str] = None):
        ...
        :return: Instance of MoveItPyConfigsBuilder with moveit_cpp loaded.
        """
        self.__moveit_configs.moveit_cpp = load_yaml(
            self._package_path / (file_path or self.__config_dir_path / "moveit_cpp.yaml")
        )
        return self
    ...
    def trajectory_execution(
        ...
        :return: Instance of MoveItPyConfigsBuilder with trajectory_execution loaded.
        """
    ...
    def planning_pipelines(
        ...
        :return: Instance of MoveItPyConfigsBuilder with planning_pipelines loaded.
        """
        config_folder = self._package_path / self.__config_dir_path
    ...
    def pilz_cartesian_limits(self, file_path: Optional[str] = None):
        ...
        :return: Instance of MoveItPyConfigsBuilder with pilz_cartesian_limits loaded.
        """
        deprecated_path = self._package_path / (self.__config_dir_path / "cartesian_limits.yaml")
    ...
    def to_moveit_configs(self):
        ...
        :return: An MoveItPyConfigs instance with all parameters loaded.
        """
